...
export function NativeArchiWorkspace() {
  ...
  async function load(file?: File) {
    ...
    try {
      ...
      const parsed = parseNativeArchi(await file.text())
      setModel(parsed)
  ...
  return <div className="flex h-full min-h-0 w-full flex-col bg-background">
    ...
    {!model ? <div className="grid flex-1 place-items-center p-8 text-center"><div className="max-w-lg">
      ...
      <aside className="flex w-60 shrink-0 flex-col border-r border-border bg-sidebar">
        <div className="space-y-2 border-b border-border p-3"><p className="truncate text-[12px] font-semibold" title={model.name}>{model.name}</p><p className="text-[11px] text-muted-foreground">{model.views.length} vistas · {model.elements.size} elementos</p>
      ...
      <section className="flex min-w-0 flex-1 flex-col">
        <div className="flex flex-wrap items-center gap-2 border-b border-border px-3 py-2">
          <span className="mr-auto text-[12px] font-semibold">{view?.name} <span className="font-normal text-muted-foreground">· {objects.length} figuras · {view?.connections.length} conexiones</span></span>
        ...
        <div className="min-h-0 flex-1 overflow-auto bg-[radial-gradient(var(--border)_0.6px,transparent_0.6px)] bg-[length:20px_20px]">
          {view && <svg width={Math.round(view.width * scale)} height={Math.round(view.height * scale)} viewBox={`0 0 ${view.width} ${view.height}`} role="img" aria-label={`Vista Archi ${view.name}`} className="block">
            ...
            {view.connections.map((c) => { const from = byId.get(c.source); const to = byId.get(c.target); if (!from || !to) return null
              return <line key={c.id} x1={from.x + from.width / 2} y1={from.y + from.height / 2} x2={to.x + to.width / 2} y2={to.y + to.height / 2} stroke="#7d97ad" strokeWidth="1.6" markerEnd="url(#native-arrow)" opacity=".75"/> })}
      ...
      <aside className="hidden w-64 shrink-0 overflow-auto border-l border-border bg-card p-4 xl:block">
        {selected ? <><h3 className="break-words text-[14px] font-semibold">{selected.label || '(sin etiqueta)'}</h3><p className="mt-2 text-[11px] text-muted-foreground">{selected.type}</p>
          <dl className="mt-4 space-y-3 text-[11px]"><div><dt className="text-muted-foreground">ID del elemento compartido</dt><dd className="break-all font-mono">{selected.elementId ?? 'Objeto visual'}</dd></div><div><dt className="text-muted-foreground">ID en esta vista</dt><dd className="break-all font-mono">{selected.id}</dd></div><div><dt className="text-muted-foreground">Vistas que reutilizan el elemento</dt><dd>{selected.elementId ? model.views.filter((v) => v.objects.some((o) => o.elementId === selected.elementId)).length : '—'}</dd></div></dl>
          <div className="mt-4 border-t border-border pt-3 text-[11px] text-muted-foreground">{selected.elementId ? model.elements.get(selected.elementId)?.documentation || 'Sin documentación en el elemento.' : 'Contenedor o figura de diagrama.'}</div>
        </> : <><h3 className="text-[14px] font-semibold">Detalle del componente</h3><p className="mt-2 text-[12px] text-muted-foreground">Seleccioná una figura del modelo. Verás su ID estable y en cuántas vistas se reutiliza, incluidas las bases locales KETAN.</p></>}
      </aside>
    </div>}
  </div>
}
